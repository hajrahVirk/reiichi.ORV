<script lang="ts">
  ...
  let {
    ...
    fileicon,
    mode = "normal",
  }: { filename: string; fileicon?: Snippet; mode?: string } = $props();
</script>

<div class="relative bottom-0">
  <div
    class="min-w-full flex justify-evenly mt-5 bg-base-content/5 pointer-events-none"
  >
    <!-- Vim Mode -->
    <span class="bg-primary light:font-bold px-5">
      <svg
        ...
        ></svg
      >
      <span id="mode">{mode.toUpperCase()}</span>
    </span>

    <div
      class="w-2 aspect-[1] bg-primary"
      style="clip-path: polygon(0 0,100% 0,0 100%);"
    ></div>

    <!-- Empty Bg -->
    <div
      class="w-2 aspect-[1] bg-base-content/20 -ml-2"
      style="
  clip-path: polygon(0 100%,100% 0,100% 100%);
"
    ></div>
    <div class="bg-base-content/20 px-2"></div>
    <div
      class="w-2 aspect-[1] bg-base-content/20"
      style="clip-path: polygon(0 0,100% 0,0 100%);"
    ></div>

    <!-- Vim File -->
    <div
      class="w-2 aspect-[1] bg-base-content/10 -ml-2"
      style="
  clip-path: polygon(0 100%,100% 0,100% 100%);
"
    ></div>
    <span class="bg-base-content/10 px-3">
      {#if !fileicon}
        <svg
          class="size-3 inline-block opacity-50"
          viewBox="0 0 15 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          ><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g
    ...
    <div
      class="w-2 aspect-[1] bg-base-content/10"
      style="clip-path: polygon(0 0,100% 0,0 100%);"
    ></div>
    ...
    <span class="mx-auto text-base-content/20">Neovim Editor</span>

    <!-- Line Position -->
    <span class="ml-auto mr-5">Ln 01, Col 01</span>

    <!-- Current Folder -->
    <span class="bg-accent text-gray-800 rounded-l-full px-2">
      <svg
        class="inline-block size-3"
        viewBox="0 0 24 24"
        fill="none"
    ...
    <span class="px-2 bg-secondary text-base-200">
      <svg
        class="inline-block size-3"
        viewBox="0 0 24 24"
        fill="none"
  ...
  <!-- <input
    disabled
    type="text"
    class="input border-none bg-base-200 rounded-none w-full h-full"
  /> -->
  <span class="w-full h-full"># currently WIP</span>
</div>
